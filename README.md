...
### Важно: это не номер канала
- `diag_error_source` не связан с номером экземпляра FB в массиве и не равен индексу канала.
- Индекс канала задается только позицией в массиве (`ch = 0..UDP_CHANNEL_COUNT-1`) и базовым адресом блока регистров.
- Один и тот же код `diag_error_source` (например `8`) может появиться на любом канале.
- Смысл `diag_error_source`: показать этап внутри конкретного FB, где возникла ошибка.

### Быстрый алгоритм диагностики
- 1. Определите канал по адресу блока (`base = ch * 48`).
- 2. Проверьте флаги: `error`, `diag_peer_error`, `diag_send_error`.
- 3. Посмотрите `diag_error_source` и определите этап ошибки по таблице `0..10`.
- 4. Для этого этапа считайте соответствующий `NBS.ERROR` код:
- `error_id` для общей ошибки FB.
- `diag_peer_error_id`, если этап связан с `UDP_Peer` (`diag_error_source = 4`).
- `diag_send_error_id`, если этап связан с `Send` (`diag_error_source = 8`).
- `diag_local_init_error` и `diag_remote_init_error`, если этап `1` или `2`.
